...
<body>
    ...
    <script>
        ...
        function beforeEditName(treeId, treeNode) {
            ...
            positionEdit(treeNode.id);
            return false;
        }
        function beforeRemove(treeId, treeNode) {
            positionDel(treeNode.id);
            return false;
        }
        ...
        function addHoverDom(treeId, treeNode) {
            ...
            var addStr = "<span class='button add' id='addBtn_" + treeNode.tId
                + "' title='添加岗位' onfocus='this.blur();'></span>";
            sObj.after(addStr);
            var btn = $("#addBtn_"+treeNode.tId);
            if (btn) btn.bind("click", function(){
                var zTree = $.fn.zTree.getZTreeObj("treeDemo");
                positionAdd(treeNode.id);
                return false;
            });
        };
        ...
        /*跳转添加页面*/
        function positionAdd(pid) {
            layer.open({
                type: 2,
                title: '添加岗位',
                content: "${baseurl}/position/addPage?pId="+pid ,
                maxmin: true,
                area: ['800px', '570px']
            });
        }
        ...
        function positionEdit(id) {
            layer.open({
                type: 2,
                title: '修改岗位',
                content: "${baseurl}/position/updatePage?id="+id ,
                maxmin: true,
                area: ['800px', '570px']
            });
        }
        ...
        function positionDel(id) {
            layer.confirm('确认要删除吗？',function(){
                $.get("${baseurl}/position/delete?ids="+id, function(data){
